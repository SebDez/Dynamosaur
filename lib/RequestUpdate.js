...
export default class RequestUpdate extends Request{
   ...
   */
  withId(ids) {
    if (!ids) throw new Exception('RU1', 'No ids provided', new Date());
    for (let key in ids) {
   ...
   */
  toModify(item) {
    if (!item) throw new Exception('RU2', 'No item provided', new Date());
  ...
  itemToExpression(item) {
    if (!item) throw new Exception('RU3', 'No item provided', new Date());
    if (!(item instanceof Object)) throw new Exception('RU4', 'The item is not an object', new Date());
    const obj = {
      UpdateExpression: null,
      ExpressionAttributeValues: {}
    };
